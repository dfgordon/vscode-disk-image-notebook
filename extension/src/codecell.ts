...
* `<json>` - JSON string describing proprietary format\n\
...
* `type [--invert] [--clear] [--break <0-255>] <file>` - display file as lossy UTF8\n\
...
* `ssfmt [<indent>]` - get soft sector format\n\
\nCode Cells - binary: \n\
...
\nNotes:\n\
* `cyl` accepts quarter-decimals for WOZ tracks\n\
...
export function parse_line(this: DiskImageController, line: string, img_path: string | undefined, img_buf: Uint8Array, notebook: vscode.NotebookDocument): Array<vscode.NotebookCellOutputItem> {
    const out_cells = new Array<vscode.NotebookCellOutputItem>();
    const maybe_method = this.method_map.get(notebook.metadata.img_hash);
    const maybe_fmt = this.fmt_map.get(notebook.metadata.img_hash);
    ...
    try {
        if (cmd.length == 1 && cmd[0] == "help") {
            ...
            const res = bin2txt(args, img_buf, maybe_method, maybe_fmt);
            out_cells.push(vscode.NotebookCellOutputItem.text(res));
        } else if (cmd.length > 0 && cmd.length < 5 && cmd[0] == "tree") {
            const res = bin2txt(cmd, img_buf, maybe_method, maybe_fmt);
            ...
            const res = bin2txt([cmd[0], "-f", file_path, "-t", "any"], img_buf, maybe_method, maybe_fmt);
            const fimg = new FileImage(res);
            const content = fimg.getText(file_path, img_buf, maybe_method, maybe_fmt)[1];
            out_cells.push(vscode.NotebookCellOutputItem.text(content));
        } else if (cmd.length == 2 && cmd[0] == "glob") {
            const content = bin2txt([cmd[0], "-f", cmd[1], "--indent", "4"], img_buf, maybe_method, maybe_fmt);
            ...
            if (valid_procs.includes(proc_arg)) {
                ...
                const file_path = img_path ? img_path + path_arg : "" + path_arg;
                const fimg_str = bin2txt(["get", "-f", file_path, "-t", "any"], img_buf, maybe_method, maybe_fmt);
                const fimg = new FileImage(fimg_str);
                const objCode = fimg.getObjectCode(file_path, img_buf, maybe_method, maybe_fmt);
                let load_addr = objCode.load_addr;
                if (load_addr == 0) {
                    load_addr = 8192; // probably a system file
                }
                const buf = Buffer.from(objCode.code, "hex");
                const content = bin2txt(["dasm", "-p", proc, "--mx", mx, "--org", load_addr.toString()], Uint8Array.from(buf), undefined, undefined);
                out_cells.push(vscode.NotebookCellOutputItem.text(content));
            } else {
            ...
            const path_arg = util.trailingArgWithSpaces(line, 1);
            const file_path = img_path ? img_path + path_arg : "" + path_arg;
            const res = bin2txt(["get", "-f", file_path, "-t", "any"], img_buf, maybe_method, maybe_fmt);
            out_cells.push(vscode.NotebookCellOutputItem.text(res, "text/x-json"));
        } else if (cmd.length > 1 && cmd[0] == "type") {
            let path_start_idx = 1;
            let xor_mask = 0x00;
            let and_mask = 0xff;
            let line_brk = null;
            for (let i = 1; i < cmd.length; i++) {
                if (cmd[i - 1] == "--break") {
                    line_brk = parseInt(cmd[i]);
                } else if (cmd[i] == "--invert") {
                    xor_mask = 0x80;
                } else if (cmd[i] == "--clear") {
                    and_mask = 0x7f;
                } else if (cmd[i] == "--break") {
            ...
            const path_arg = util.trailingArgWithSpaces(line, path_start_idx);
            const file_path = img_path ? img_path + path_arg : "" + path_arg;
            const res = bin2bin(["get", "-f", file_path, "-t", "bin"], img_buf, maybe_method, maybe_fmt);
            for (let i = 0; i < res.length; i++) {
                res[i] = res[i] ^ xor_mask;
                res[i] = res[i] & and_mask;
                if (res[i] == line_brk) {
                    res[i] = 0x0a;
                }
            }
            const lossyText = Buffer.from(res).toString();
            ...
            const res = bin2txt(["get", "-f", file_path, "-t", "any"], img_buf, maybe_method, maybe_fmt);
            const fimg = new FileImage(res);
            const res2 = fimg.getHex(file_path, img_buf, true, maybe_method, maybe_fmt);
            out_cells.push(vscode.NotebookCellOutputItem.text(res2));
        } else if (cmd.length == 1 && cmd[0] == "meta") {
            const res = bin2txt(["get", "-t", "meta"], img_buf, maybe_method, maybe_fmt);
            out_cells.push(vscode.NotebookCellOutputItem.text(res, "text/x-json"));
        } else if (cmd.length == 2 && cmd[0] == "stat") {
            const indent = parseInt(cmd[1]);
            if (!isNaN(indent)) {
                const res = bin2txt(["stat", "--indent", indent.toString()], img_buf, maybe_method, maybe_fmt);
                out_cells.push(vscode.NotebookCellOutputItem.text(res, "text/x-json"));
            } else {
                out_cells.push(vscode.NotebookCellOutputItem.text("argument did not parse as integer"));
            }
        } else if (cmd.length == 1 && cmd[0] == "stat") {
            const res = bin2txt(["stat"], img_buf, maybe_method, maybe_fmt);
            out_cells.push(vscode.NotebookCellOutputItem.text(res, "text/x-json"));
        } else if (cmd.length == 2 && cmd[0] == "geometry") {
            const indent = parseInt(cmd[1]);
            if (!isNaN(indent)) {
                const res = bin2txt(["geometry", "--indent", indent.toString()], img_buf, maybe_method, maybe_fmt);
                out_cells.push(vscode.NotebookCellOutputItem.text(res, "text/x-json"));
            } else {
                out_cells.push(vscode.NotebookCellOutputItem.text("argument did not parse as integer"));
            }
        } else if (cmd.length == 1 && cmd[0] == "geometry") {
            const res = bin2txt(["geometry"], img_buf, maybe_method, maybe_fmt);
            out_cells.push(vscode.NotebookCellOutputItem.text(res, "text/x-json"));
        } else if (cmd.length == 2 && cmd[0] == "ssfmt") {
            const indent = parseInt(cmd[1]);
            if (!isNaN(indent)) {
                const res = bin2txt(["geometry", "--abstract", "--indent", indent.toString()], img_buf, maybe_method, maybe_fmt);
                out_cells.push(vscode.NotebookCellOutputItem.text(res, "text/x-json"));
            } else {
                out_cells.push(vscode.NotebookCellOutputItem.text("argument did not parse as integer"));
            }
        } else if (cmd.length == 1 && cmd[0] == "ssfmt") {
            const res = bin2txt(["geometry", "--abstract"], img_buf, maybe_method, maybe_fmt);
            out_cells.push(vscode.NotebookCellOutputItem.text(res, "text/x-json"));
            ...
            const res = bin2txt(["get", "--console", "-t", "track", "-f", cmd[1]], img_buf, maybe_method, maybe_fmt);
            out_cells.push(vscode.NotebookCellOutputItem.text(res));
            ...
            const res = bin2txt(["get", "-t", "sec", "-f", seq, "--console"], img_buf, maybe_method, maybe_fmt);
            out_cells.push(vscode.NotebookCellOutputItem.text(res));
        } else if (cmd.length == 2 && cmd[0] == "block") {
            const res = bin2txt(["get", "-t", "block", "-f", cmd[1], "--console"], img_buf, maybe_method, maybe_fmt);
            out_cells.push(vscode.NotebookCellOutputItem.text(res));
